...
class EdicaoUCControllerIntegrationTests
{
    ...
    {
        EdicaoUCAlunoDTO dto = new EdicaoUCAlunoDTO(1L, 1L);
        when(service.addAlunoEdicaoUC(1L, 1L, 1L)).thenReturn(dto);

        MvcResult response = mockMvc
                .perform(MockMvcRequestBuilders.post("/edicaoUC/inscrever/1L?alunoID=1L")
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andReturn();

        EdicaoUCAlunoDTO responseDto = objectMapper.readValue(response.getResponse().getContentAsString(),
                EdicaoUCAlunoDTO.class);
        assertEquals(dto.toString(), responseDto.toString());
    }

    @Test
    @WithMockUser(username = "docente", password = "password", authorities = "ROLE_DOCENTE")
    public void shouldNotReturnEdicaoUCAlunoDTO_InvalidAtributtes() throws Exception
    {
        when(service.addAlunoEdicaoUC(1L, 1L, 1L)).thenThrow(ErroGeralException.class);
    ...
    @Test
    @WithMockUser(username = "docente", password = "password", authorities = "ROLE_DOCENTE")
    public void shouldNotReturnEdicaoUCAlunoDTO_InvalidCredentials() throws Exception
    {
        when(service.addAlunoEdicaoUC(1L, 1L, 1L)).thenThrow(ErrorDetail.class);
    ...
    {
        EdicaoUCDTO dto = new EdicaoUCDTO("sigla", "denominacao", 1L);
        when(service.activarEdicao(1L, 1L)).thenReturn(dto);

        MvcResult response = mockMvc
                .perform(MockMvcRequestBuilders.patch("/edicaoUC/ativarEdicao/{edicaoUCID}", 1L)
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andReturn();
    ...
    {
        EdicaoUCDTO dto = new EdicaoUCDTO("sigla", "denominacao", 1L);
        when(service.activarEdicao(1L, 1L)).thenThrow(OptionalVazioException.class);
    ...
    @Test
    @WithMockUser(username = "docente", password = "password", authorities = "ROLE_DOCENTE")
    public void shouldNotReturnEdicaoUCActivated_InvalidEdicaoFase() throws Exception
    {
        EdicaoUCDTO dto = new EdicaoUCDTO("sigla", "denominacao", 1L);
        when(service.activarEdicao(1L, 1L)).thenThrow(ValidacaoInvalidaException.class);
